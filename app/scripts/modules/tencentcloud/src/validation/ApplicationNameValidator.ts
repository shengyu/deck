...
class TencentCloudApplicationNameValidator implements IApplicationNameValidator {
  ...
  private validateClassicLock(warnings: string[]): void {
    ...
    if (lockoutDate && lockoutDate < new Date().getTime()) {
      warnings.push(
        `New applications deployed to Tencent Cloud are restricted to VPC; you cannot create server groups,
  ...
  private validateLength(name: string, warnings: string[], errors: string[]) {
    if (name.length > 250) {
      errors.push('The maximum length for an application in Tencent Cloud is 250 characters.');
      return;
    }
    if (name.length > 240) {
      if (name.length >= 248) {
    ...
    if (name.length > 20) {
      if (name.length > 32) {
        warnings.push(`You will not be able to create an Tencent Cloud load balancer for this application if the
